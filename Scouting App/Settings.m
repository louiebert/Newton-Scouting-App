...
NSInteger matchNumber = 0;
NSInteger matchNumberEntered = 0;
NSInteger iPadNumber = 0;
NSString *scoutName;
NSString *matchNumberString;
NSInteger currentTeamNumber = 0;
...
- (IBAction)saveBtn:(id)sender {
    ...
    iPadNumber = _whichIPadSeg.selectedSegmentIndex;
    matchNumberString = [_qualMatchTextField text];
    matchNumberEntered = [matchNumberString intValue];
    matchNumber = matchNumberEntered;
    scoutName = _scoutNameTextField.text;
    if (matchNumberEntered < 1 || matchNumberEntered>134) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle: @"BAD MATCH NUMBER"
                                                       message: @"Please enter a valid match number for this regional."
                                                      delegate: self
                                             cancelButtonTitle:@"OK"
                                             otherButtonTitles:nil];
        [alert show];
    }
    else if ([scoutName isEqual: @""]) {
    ...
    if (matchNumberEntered >= 1 && matchNumberEntered<=134 && ![scoutName isEqual:@""]){
        if (matchNumber == 0){
            _teamNumber.text = [[NSString alloc] initWithFormat:@"????"];
            _matchNumberLabel.text = [[NSString alloc] initWithFormat:@"??"];
        }
        else{
            if(iPadNumber == 0){
                _teamNumber.text = [[NSString alloc] initWithFormat: @"%@", [redOneTeams objectAtIndex:
                                                                         (matchNumber-1)]];
                _teamNumber.textColor = [UIColor redColor];
            }
            if(iPadNumber == 1){
                _teamNumber.text = [[NSString alloc] initWithFormat: @"%@", [redTwoTeams objectAtIndex:
                                                                             (matchNumber-1)]];
                _teamNumber.textColor = [UIColor redColor];
            }
            if(iPadNumber == 2){
                _teamNumber.text = [[NSString alloc] initWithFormat: @"%@", [redThreeTeams objectAtIndex:
                                                                             (matchNumber-1)]];
                _teamNumber.textColor = [UIColor redColor];
            }
            if(iPadNumber == 3){
                _teamNumber.text = [[NSString alloc] initWithFormat: @"%@", [blueOneTeams objectAtIndex:
                                                                             (matchNumber-1)]];
                    _teamNumber.textColor = [UIColor blueColor];
            }
            if(iPadNumber == 4){
                _teamNumber.text = [[NSString alloc] initWithFormat: @"%@", [blueTwoTeams objectAtIndex:
                                                                             (matchNumber-1)]];
                _teamNumber.textColor = [UIColor blueColor];
            }
            if(iPadNumber == 5){
                _teamNumber.text = [[NSString alloc] initWithFormat: @"%@", [blueThreeTeams objectAtIndex:
                                                                             (matchNumber-1)]];
                _teamNumber.textColor = [UIColor blueColor];
            }
            _matchNumberLabel.text = [[NSString alloc] initWithFormat: @"%d", matchNumber];
        }
    }
}
